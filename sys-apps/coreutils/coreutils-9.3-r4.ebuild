...
if [[ ${PV} == 9999 ]] ; then
	...
	"
#   Unkeyworded to prevent poor innocent souls from breaking coreutils
...
PDEPEND="app-alternatives/coreutils"

pkg_pretend() {
	ewarn "Please make sure /usr/bin/env is in UNINSTALL_IGNORE prior to installing. Otherwise, migrating from sys-apps/coreutils::gentoo will break your system completely. You have been warned!"
}
...
src_install() {
	...
	export COREUTILS_BINS=( "${ED}/usr/bin/"* )
	export COREUTILS_MANPAGES=( "${ED}/usr/share/man/man1/"* )
	...
	if use split-usr ; then
		...
		mv ${fhs}-gnu ../../bin/ || die "Could not move FHS bins!"

		if use hostname ; then
			mv hostname-gnu ../../bin/ || die
		fi
		...
		mv ${com}-gnu ../../bin/ || die "Could not move common bins!"
...
pkg_postinst() {
	local l
	for l in $(qlist sys-apps/coreutils | grep -F /bin/) ; do
		if [[ -e ${l} ]] && ! [[ -e ${l/-gnu/} ]] ; then
			ln-gnu -s "${l##*/}" "${l/-gnu/}" || die
		fi
	done
