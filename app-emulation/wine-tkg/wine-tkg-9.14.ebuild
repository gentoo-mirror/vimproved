...
if [[ ${PV} == *9999 ]]; then
	...
	WINE_TKG_COMMIT="f2039da837253205ee6727519570d030ee4a9eb8"
	SRC_URI="
		https://dl.winehq.org/wine/source/${WINE_SDIR}/${WINE_P}.tar.xz
		https://github.com/wine-staging/wine-staging/archive/v${PV}.tar.gz -> wine-staging-${PV}.tar.gz
		https://github.com/Frogging-Family/wine-tkg-git/archive/${WINE_TKG_COMMIT}.tar.gz -> ${P}.tar.gz"
	KEYWORDS="-* ~amd64 ~x86"
fi
...
src_prepare() {
	...
	edo "${PYTHON}" ../wine-staging-${PV}/staging/patchinstall.py "${patchinstallargs[@]}"
	...
	# To update this list, run non-makepkg-build.sh and inspect prepare.log
	...
	local TKG_PATCHES=(
		...
		"${WINE_TKG_PATCHDIR}/hotfixes/proton_fs_hack_staging/remove_hooks_that_time_out2.mypatch"
		"${WINE_TKG_PATCHDIR}/hotfixes/GetMappedFileName/Return_nt_filename_and_resolve_DOS_drive_path.mypatch"
		"${WINE_TKG_PATCHDIR}/hotfixes/08cccb5/a608ef1.mypatch"
		"${WINE_TKG_PATCHDIR}/hotfixes/autoconf-opencl-hotfix/opencl-fixup.mypatch"
		"${WINE_TKG_PATCHDIR}/hotfixes/NosTale/nostale_mouse_fix.mypatch"
		"${WINE_TKG_PATCHDIR}/misc/wine-tkg/wine-tkg-staging.patch"
	)

	einfo "Applying TKG patches"
	eapply "${TKG_PATCHES[@]}"
	...
	if tc-is-clang; then
		...
		if ! use mingw; then
			...
			[[ ${EXTRA_ECONF} == *--enable-archs* ]] ||
				ewarn "building ${PN} with clang is only supported with USE=mingw"
		fi
	fi
