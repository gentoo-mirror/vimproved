...
inherit crossdev flag-o-matic llvm toolchain-funcs prefix
if [[ ${PV} == "9999" ]] ; then
...
SLOT="0"
IUSE="crypt headers-only scudo split-usr"
...
BDEPEND="
	${BDEPEND}
	scudo? ( <sys-devel/clang-$(( LLVM_MAX_SLOT + 1 )):= )
"

PATCHES=(
	"${FILESDIR}/${PN}-1.2.4-scudo-plumbing.patch"
	"${FILESDIR}/${PN}-1.2.4-scudo-clean-symbols.patch"
)
...
src_prepare() {
	mkdir -p "${S}/src/malloc/scudo/scudo" || die
...
src_configure() {
	...
	append-cppflags "-I/usr/lib/clang/${LLVM_SLOT}/include"
	...
	./configure \
		...
		--disable-gcc-wrapper \
		--with-malloc=$(usex scudo scudo mallocng) || die
}
